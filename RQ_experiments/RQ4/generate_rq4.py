...
def plot_1x2_size(df_lcb, df_cxg, lcb_data, cxg_data, out_path, xlabel, ylabel, log_x=False):
    ...
    lcb_xmin, lcb_xmax, lcb_classes, lcb_cx, lcb_cy = lcb_data
    cxg_xmin, cxg_xmax, cxg_classes, cxg_cx, cxg_cy = cxg_data
    
    y_col = "acc_values" if "Accuracy" in ylabel else "energy_consumed"
    x_col = "log_size_gb" if log_x else "size_gb"
    
    plots = [
        (axes[0], df_lcb, lcb_data, "LiveCodeBench"),
        (axes[1], df_cxg, cxg_data, "CodeXGLUE")
    ]
    
    im = None
    for ax, df, data, title in plots:
...
def generate_rq4(data_file, report_dir):
    ...
    # 1. Size vs Accuracy (Capability Density)
    print("Processing Capability Density (Size vs Acc)...")
    df_lcb_acc, lcb_acc_data = run_size_metric(data_file, "livecodebench", LogAccPowerLaw, "acc_values", False)
    df_cxg_acc, cxg_acc_data = run_size_metric(data_file, "code2text_python", LogAccPowerLaw, "acc_values", False)
    ...
    # 2. Size vs Energy (Structural Efficiency)
    print("Processing Structural Efficiency (Size vs Ene)...")
    df_lcb_ene, lcb_ene_data = run_size_metric(data_file, "livecodebench", LogEnergyPowerLaw, "energy_consumed", True)
    df_cxg_ene, cxg_ene_data = run_size_metric(data_file, "code2text_python", LogEnergyPowerLaw, "energy_consumed", True)
